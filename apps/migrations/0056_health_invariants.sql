-- Migration: 0056_health_invariants
--
-- Adds public.health_invariants() — a set of data-consistency checks that catch
-- silent corruption regardless of which code path caused it.
--
-- WHY THIS EXISTS
--   Feature health has been verified by "can a human see it on an admin page?",
--   which is not a signal — it is the possibility of a signal. Several known
--   failure paths log to console and nothing else. The clearest example is
--   /api/bookings/confirm-free, which on a failed $0-payment insert emits
--   "CRITICAL: $0 payment record insert failed" to console.error and returns
--   success to the customer. Nothing surfaces that. Check #2 below does.
--
-- DESIGN
--   * One function, one round trip, returns EVERY check with its breach count —
--     not just the failures. A canary that goes quiet when healthy is
--     indistinguishable from a canary that died. "12 checks, 0 breaches" is the
--     signal that the canary itself ran.
--   * Grace windows on anything with an async follow-up step, so a row observed
--     mid-write is not reported as corruption.
--   * SECURITY DEFINER because it reads across every table irrespective of RLS.
--     Execute is REVOKED FROM PUBLIC — revoking from anon/authenticated alone is
--     a silent no-op, since PUBLIC is what actually carries the default grant.
--   * search_path is pinned (see migration 0047) so the function cannot be
--     hijacked by a caller-controlled search_path.
--
-- CONSUMED BY
--   /api/admin/daily-summary (migration 0053 cron, 11:00 UTC, production only)
...
-- ── Lock down execute ────────────────────────────────────────────────────────
-- PUBLIC carries the default EXECUTE grant on new functions. Revoking from
-- anon/authenticated without first revoking from PUBLIC is a silent no-op.
...
-- ── Supporting indexes ───────────────────────────────────────────────────────
-- Checks 2 and 11 scan bookings by session and by payment presence.
create index if not exists idx_bookings_active_session
  on bookings (session_id) where cancelled = false;

-- Check 2's NOT EXISTS probe against payments.
create index if not exists idx_payments_booking_completed
  on payments (booking_id) where status = 'completed';

-- Check 12's webhook recency probe.
