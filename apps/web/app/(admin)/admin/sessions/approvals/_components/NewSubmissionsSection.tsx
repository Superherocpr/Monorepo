...
/**
 * Renders the new submissions section with per-card inline actions and an Approve All button.
 ...
 */
export default function NewSubmissionsSection({
  sessions,
}: NewSubmissionsSectionProps): React.ReactElement | null {
  ...
  /**
   * Approves all sessions still in "idle" status in a single batch request.
  ...
  async function handleApproveAll(): Promise<void> {
    ...
    if (err) {
      setBulkError(err);
      // Reset loading cards to idle so the manager can retry individually.
      setStatuses((prev) => ({
        ...prev,
        ...Object.fromEntries(
  ...
  return (
    <section>
      ...
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-semibold text-gray-900">New Submissions</h2>
        {idleCount > 1 && (
          <button
            type="button"
