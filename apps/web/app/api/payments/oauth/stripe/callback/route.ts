/**
 * GET /api/payments/oauth/stripe/callback
 * Called by: Stripe — redirected here after instructor grants consent
 * Auth: must be a logged-in instructor or super_admin
 * Exchanges the authorization code for tokens via Stripe Connect,
 * stores encrypted tokens in instructor_payment_accounts, redirects to payment settings.
 * The Stripe token response includes stripe_user_id as the platform account identifier.
 */
...
export async function GET(request: Request) {
  ...
  const storedState = cookieStore.get("oauth_state_stripe")?.value;
  cookieStore.delete("oauth_state_stripe");
  ...
  // ── Exchange code for tokens via Stripe Connect ────────────────────────────
  ...
  // stripe_user_id is the connected account identifier (e.g. acct_1234...)
  const platformAccountId = tokenData.stripe_user_id ?? null;
  ...
  redirect("/admin/profile/payment?connected=stripe");
}
